...
class DQN(nn.Module):
    ...
    def __init__(self, numberOfInputs, numberOfOutputs, numberOfNeurons, dropout):
        ...
        # Define CNN for data preprocessing
        self.cnn1 = nn.Conv2d(1, 8, kernel_size=(1,4), stride=1)
        self.cnn2 = nn.Conv2d(8, 8, kernel_size=(3,1), stride=1)
        self.cnn3 = nn.Conv2d(8, 8, kernel_size=(3, 1), stride=1)

        self.pool = MaxPool2d(kernel_size=(2, 1), stride=(2, 1))

        # Definition of some Fully Connected layers
        self.fc1 = nn.Linear(40, numberOfOutputs)
    ...
    def forward(self, input):
        ...
        output = self.fc1(self.dropout1(x))
        return output
...
class TCNNDQN:
    ...
    def __init__(self, observationSpace, actionSpace, run_config):
        ...
        self.writer = SummaryWriter('runs/' + run_time,
                                    comment='strategy: TCNNDQN')
        self.writer.add_text("TCNNDQN" + run_time, "Model-parameters: \n" + str(model_params) + "\n \n"
    ...
    def processState(self, state, coefficients):
        ...
        # Process the state structure to obtain the appropriate format for CNN according to paper
        return np.array(state[:-1]).transpose()
    ...
    def training(self, trainingEnv, trainingParameters=[],
        ...
        dataAugmentation = DataAugmentation(filterRange=[self.filterOrder])
        ...
        if showPerformance:
            analyser = PerformanceEstimator(trainingEnv.data)
            analyser.displayPerformance('TCNNDQN')
